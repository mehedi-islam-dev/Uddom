      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {[
              { key: 'nameEn', label: t('student_name_en') },
              { key: 'nameBn', label: t('student_name_bn') },
              { key: 'batchEn', label: t('student_batch_en') },
              { key: 'batchBn', label: t('student_batch_bn') },
              { key: 'rollNumber', label: t('student_roll') },
            ].map(({ key, label }) => (
              <div key={key}>
                <label className="block text-sm font-medium text-gray-700 mb-1.5" htmlFor={`student-${key}`}>{label}</label>
                <input id={`student-${key}`} type="text" value={data[key as keyof typeof data]} onChange={(e) => set(key, e.target.value)} required className={inputClass} />
              </div>
            ))}
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-3">
              <input ref={fileInputRef} type="file" accept="image/*" onChange={handleFileChange} className="hidden" id="student-image-upload" disabled={uploading} />
              <label htmlFor="student-image-upload" className={`flex items-center gap-2 px-4 py-2.5 rounded-xl border text-sm font-medium cursor-pointer transition-all duration-200 ${uploading ? 'border-indigo-200 bg-indigo-50 text-indigo-400 cursor-not-allowed' : 'border-indigo-200 text-indigo-700 hover:bg-indigo-50 hover:border-indigo-300'}`}>
