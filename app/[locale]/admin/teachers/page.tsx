...
export default function ManageTeachersPage() {
  ...
  const [editingTeacher, setEditingTeacher] = useState<TeacherData | undefined>();
  ...
  const handleSuccess = (teacher: TeacherData) => {
    setTeachers((prev) => {
      const exists = prev.find((t) => t._id === teacher._id);
      if (exists) {
        return prev.map((t) => (t._id === teacher._id ? teacher : t));
      }
      return [...prev, teacher];
    });
    setShowForm(false);
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-extrabold text-gray-900">{t('manage_teachers')}</h1>
          <p className="text-gray-500 text-sm mt-1">{teachers.length} teachers registered</p>
        </div>
        <button
          onClick={() => { setEditingTeacher(undefined); setShowForm(true); }}
      ...
      {loading ? (
        ...
        <div className="text-center py-24">
          ...
          <p className="text-gray-500">No teachers yet. Add your first teacher!</p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                ...
                <th className="text-left px-5 py-3.5 font-semibold text-gray-600 hidden md:table-cell">Subject</th>
                <th className="text-left px-5 py-3.5 font-semibold text-gray-600 hidden lg:table-cell">Experience</th>
                ...
                <tr key={teacher._id} className="hover:bg-gray-50/50 transition-colors">
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-semibold text-gray-900 leading-tight">{teacher.nameEn}</p>
                        <p className="text-gray-400 text-xs">{teacher.nameBn}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-5 py-4 hidden md:table-cell">
                    <div>
                      <p className="text-gray-700">{teacher.subjectEn}</p>
                      <p className="text-gray-400 text-xs">{teacher.subjectBn}</p>
                    </div>
                  </td>
                  <td className="px-5 py-4 text-gray-600 hidden lg:table-cell">{teacher.experience}</td>
      ...
      {showForm && (
        <TeacherForm
          ...
          onCancel={() => { setShowForm(false); setEditingTeacher(undefined); }}
        />
      )}
    </div>
  );
